...
export interface PdfGenerationOptions {
  ...
  /**
   * Optional deterministic file path for saving PDF.
   * If provided, PDF will be saved to this path without showing save dialog.
   */
...
class PdfService {
  ...
  async generateHTMLTemplate(options: PdfGenerationOptions): Promise<string> {
    ...
    try {
      ...
      if (
        productNameRaw.includes("Participating Whole Life") ||
        productNameRaw.includes("PWL")
      ) {
        productAbbrev = "PWL";
        productType = "Participating Whole Life Insurance";
      } else if (productNameRaw.includes("Term")) {
        productAbbrev =
          getProductShortCode(productNameRaw) ||
          productNameRaw.split(" - ")[0] ||
          productNameRaw;
      ...
      console.warn(
        "[PdfService] Failed to get product info, using defaults:",
        error
      );
    ...
    const smokingStatus =
      quote.insured?.smokingHabit || quote.smokingHabit || "Non-smoker";
    ...
    if (!calculatedPremium || calculatedPremium === 0) {
      if (
        productNameForIllustration === "Flexible Premium Annuity" ||
        productNameForIllustration === "NFL Annuity"
      ) {
        calculatedPremium = baseAmountForIllustration;
      } else {
        try {
          const premiumParams = {
            productType: getProductShortCode(
            ...
            paymentMethod: (quote.paymentMethod === "Regular" ||
            quote.paymentMethod === "R"
          ...
          console.warn(
            "[PdfService] Failed to calculate premium, using 0:",
            error
          );
          calculatedPremium = 0;
        }
    ...
    const monthlyPremiumConverted = convertToMonthly(
      monthlyPremium,
      paymentModeStr
    );
    ...
    if (planCodeForIllustration && !isTermProduct(productNameForIllustration)) {
      try {
        ...
        const year5Data = await AccumulatedPUA(
        ...
        const year10Data = await AccumulatedPUA(
          baseAmountForIllustration,
          10,
          planCodeForIllustration,
          sex,
        ...
        const year20Data = await AccumulatedPUA(
          baseAmountForIllustration,
          20,
          planCodeForIllustration,
          sex,
        ...
        if (yearsTo70 > 0) {
          const age70Data = await AccumulatedPUA(
            baseAmountForIllustration,
            yearsTo70,
            planCodeForIllustration,
            sex,
        ...
        if (
          ...
          const finalYearData = await AccumulatedPUA(
            baseAmountForIllustration,
            finalYear,
            planCodeForIllustration,
            sex,
        ...
        console.warn(
          "[PdfService] Failed to calculate illustration data for Summary Years:",
          error
        );
      }
      ...
      try {
        ...
        if (yearsTo70 > 0 && planCodeForIllustration) {
          const age70Data = await calculatePremierIllustration(
            baseAmountForIllustration,
            yearsTo70,
            planCodeForIllustration,
            sex,
        ...
        console.warn(
          "[PdfService] Failed to calculate Premier illustration:",
          error
        );
      }
    } else if (
      planCodeForIllustration &&
      isTermProductByPlanCode(planCodeForIllustration)
    ) {
      // Handle Term products
      try {
        const productType = getProductShortCode(
          productNameForIllustration
        ) as ProductType;
        const convertedPaymentMode = (quote.paymentMode ||
          "Monthly") as PaymentMode;
        const convertedPaymentMethod = (
          quote.paymentMethod === "Regular" || quote.paymentMethod === "R"
            ? PaymentMethod.Regular
            : PaymentMethod.EFT
        ) as PaymentMethod;

        // Helper function to calculate total premiums for a given number of years
        const calculateTermPremiums = async (
          years: number
        ): Promise<number> => {
          let totalPremiums = 0;
          for (let year = 1; year <= years; year++) {
            try {
              ...
              console.log(
                `[PdfService] Failed to calculate premium for term year ${year}:`,
                error
              );
            }
          }
        ...
        console.warn(
          "[PdfService] Failed to calculate term product illustration:",
          error
        );
      }
    }
    ...
    const isTermProductForPdf = planCodeForIllustration
      ? isTermProductByPlanCode(planCodeForIllustration)
      : false;
    ...
    try {
      if (
        ...
        planCodeForIllustration &&
        !isTermProduct(productNameForIllustration)
      ) {
      ...
      console.warn(
        "[PdfService] Failed to generate yearly illustration data:",
        error
      );
    }
    ...
    const generateTableRows = (
      dataToShow: typeof yearlyIllustrationData
    ): string => {
      const rows: string[] = [];
      let previousYear = 0;
      ...
      dataToShow.forEach((row, index) => {
        if (
          ...
          rows.push(
            "<tr>" +
              '<td style="height: 10px; border-left: 1px solid #000; border-right: 1px solid #000;"></td>'.repeat(
                10
              ) +
              "</tr>"
          );
        ...
        rows.push(
          "<tr>" +
            ...
            "</td>" +
            "<td>" +
            formatNumber(row.contractPremium) +
            "</td>" +
            "<td>" +
            formatNumber(row.guaranteedCashValue) +
            "</td>" +
            "<td>" +
            formatNumber(row.guaranteedDeathBenefit) +
            "</td>" +
            "<td>" +
            formatNumber(row.annualDividend) +
            "</td>" +
            "<td>" +
            formatNumber(row.accumPaidUpAdditions) +
            "</td>" +
            "<td>" +
            formatNumber(row.currentCashValue) +
            "</td>" +
            "<td>" +
            formatNumber(row.currentDeathBenefit) +
            "</td>" +
            "<td>" +
            formatNumber(row.totalPaidUp) +
            "</td>" +
            "</tr>"
        );
      });
    ...
    const filteredIllustrationData = isTermProductForPdf
      ? yearlyIllustrationData.filter(
          ({ contractPremium }) => contractPremium > 0
        )
      : yearlyIllustrationData;

    const page5Data = filteredIllustrationData.filter(
      ({ endOfYear }) => endOfYear <= 40
    );
    const page6Data = filteredIllustrationData.filter(
      ({ endOfYear }) => endOfYear > 40
    );
  ...
  private async getHTMLTemplate(params: any): Promise<string> {
    try {
      ...
      throw new Error(
        `Failed to generate HTML template: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  private async convertImageToBase64(
    ...
    try {
      ...
      if (!isElectron) {
        console.warn(
          "[PdfService] Not in Electron, cannot convert image to base64"
        );
        return null;
      }
  ...
  async generatePdf(options: PdfGenerationOptions): Promise<string | null> {
    try {
      console.log("[PdfService] Generating PDF with options:", {
        ...
        insuredName:
          `${options.insuredFirstName || ""} ${options.insuredLastName || ""}`.trim() ||
          "N/A",
      ...
      if (
        companyLogoUri &&
        (companyLogoUri.startsWith("/") || companyLogoUri.startsWith("./"))
      ) {
        ...
        if (base64Logo) {
          ...
          console.warn(
            "[PdfService] Failed to convert logo to base64, using original path"
          );
        }
      }
      ...
      // If deterministicPath is provided, save directly without dialog
      if (options.deterministicPath) {
        console.log(
          "[PdfService] Using deterministic path:",
          options.deterministicPath
        );
        ...
        if (!pdfBuffer) {
          console.warn("[PdfService] PDF generation failed");
          return null;
        }

        // Save to deterministic path
        const filePath = await savePDFFileToPath(
          pdfBuffer,
          options.deterministicPath
        );

        if (!filePath) {
          console.warn("[PdfService] Failed to save PDF to deterministic path");
          return null;
        }

        console.log("[PdfService] PDF saved successfully to:", filePath);
        return filePath;
      }
