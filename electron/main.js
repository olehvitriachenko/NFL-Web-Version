...
function createWindow() {
    ...
    win.webContents.loadURL = function (url, options) {
    ...
    win.loadFile = function (filePath, options) {
...
function handleProtocolUrl(url) {
    ...
    try {
        ...
        if (callbackData.code)
            params.set('code', callbackData.code);
        if (callbackData.state)
            params.set('state', callbackData.state);
        if (callbackData.error)
            params.set('error', callbackData.error);
        if (callbackData.errorDescription)
            params.set('error_description', callbackData.errorDescription);
...
ipcMain.handle('pdf:convertImageToBase64', async (_, imagePath) => {
    try {
        const isDev = process.env.NODE_ENV === 'development' || !app.isPackaged;
...
ipcMain.handle('rates:getDatabaseVersion', async (_, accessToken) => {
    try {
...
ipcMain.handle('rates:downloadDatabase', async (_, accessToken) => {
    try {
        const BASE_URL = process.env.VITE_BACKEND_API_URL || 'https://nfl-api.test.emorydevelopment.com';
...
ipcMain.handle('rates:updateDatabase', async (_, accessToken) => {
    try {
